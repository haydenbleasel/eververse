...
export const POST = async (request: Request): Promise<Response> => {
  ...
  if (!content) {
    return new Response('Changelog content not found', { status: 404 });
  }
  ...
  try {
    // Generate embedding for the changelog content
    const embedding = await generateEmbedding(fullText);

    // Update the changelog record with the vector embedding
